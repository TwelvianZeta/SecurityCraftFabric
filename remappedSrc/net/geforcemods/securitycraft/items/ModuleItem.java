...
//				}
	...
	public void appendTooltip(ItemStack stack, World world, List<Text> list, TooltipContext flag) {
		...
		if(nbtCanBeModified && stack.getNbt() != null && !stack.getNbt().isEmpty()) {
			list.add(LiteralText.EMPTY);
			list.add(new TranslatableText("tooltip.securitycraft:module.playerCustomization.players").setStyle(GRAY_STYLE));

			for(int i = 1; i <= MAX_PLAYERS; i++)
				if(!stack.getNbt().getString("Player" + i).isEmpty())
					list.add(new LiteralText(stack.getNbt().getString("Player" + i)).setStyle(GRAY_STYLE));
		}
		...
		if(canBeCustomized()) {
			...
			if(getNumberOfAddons() > 0 && !getAddons(stack.getNbt()).isEmpty()) {
				list.add(LiteralText.EMPTY);

				list.add(ClientUtils.localize("tooltip.securitycraft:module.itemAddons.added").setStyle(GRAY_STYLE));

				for(ItemStack addon : getAddons(stack.getNbt()))
	...
	public ArrayList<Block> getBlockAddons(NbtCompound tag){
		...
		for(int i = 0; i < items.size(); i++) {
			...
			if(slot < numberOfBlockAddons) {
				...
				if((stack = ItemStack.fromNbt(item)).getItem() instanceof BlockItem)
	...
	public ArrayList<ItemStack> getAddons(NbtCompound tag){
		...
		for(int i = 0; i < items.size(); i++) {
			...
			if(slot < numberOfBlockAddons)
				list.add(ItemStack.fromNbt(item));
		}
