...
//						SecurityCraft.proxy.displayMRATGui(stack); // TODO
					...
					stack.getNbt().putIntArray(("mine" + availSlot), BlockUtils.fromPos(pos));
...
//			SecurityCraft.proxy.displayMRATGui(stack);
	...
	public void appendTooltip(ItemStack stack, World world, List<Text> list, TooltipContext flag) {
		...
		for(int i = 1; i <= 6; i++)
			if(stack.getNbt().getIntArray("mine" + i).length > 0){
				int[] coords = stack.getNbt().getIntArray("mine" + i);
		...
		for(int i = 1; i <= 6; i++)
			if(stack.getNbt().getIntArray("mine" + i).length > 0){
				...
				if(coords[0] == pos.getX() && coords[1] == pos.getY() && coords[2] == pos.getZ()){
					stack.getNbt().putIntArray("mine" + i, new int[]{0, 0, 0});
//					SecurityCraft.channel.send(PacketDistributor.PLAYER.with(() -> (ServerPlayerEntity)player), new UpdateNBTTagOnClient(stack)); // TODO
					return;
				}
			}
	...
	private int getNextAvaliableSlot(ItemStack stack){
		for(int i = 1; i <= 6; i++)
			if(stack.getNbt() == null)
				return 1;
			else if(stack.getNbt().getIntArray("mine" + i).length == 0 || (stack.getNbt().getIntArray("mine" + i)[0] == 0 && stack.getNbt().getIntArray("mine" + i)[1] == 0 && stack.getNbt().getIntArray("mine" + i)[2] == 0))
				return i;
			else
				continue;
