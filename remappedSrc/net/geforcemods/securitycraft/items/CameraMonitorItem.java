...
//	}
	...
	public void appendTooltip(ItemStack stack, World world, List<Text> tooltip, TooltipContext flag) {
		...
		tooltip.add(ClientUtils.localize("tooltip.securitycraft:cameraMonitor").append(new LiteralText(" " + getNumberOfCamerasBound(stack.getNbt()) + "/30")).setStyle(GRAY_STYLE));
	}

	public static String getTagNameFromPosition(NbtCompound tag, CameraView view) {
		for(int i = 1; i <= 30; i++)
	...
	public boolean hasCameraAdded(NbtCompound tag){
	...
	public boolean isCameraAdded(NbtCompound tag, CameraView view){
		for(int i = 1; i <= 30; i++)
			if(tag.contains("Camera" + i)){
				String[] coords = tag.getString("Camera" + i).split(" ");
	...
	public ArrayList<CameraView> getCameraPositions(NbtCompound tag){
	...
	public int getNumberOfCamerasBound(NbtCompound tag) {
