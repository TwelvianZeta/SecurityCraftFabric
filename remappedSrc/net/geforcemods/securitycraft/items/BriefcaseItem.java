...
//	}
	...
	public void appendTooltip(ItemStack briefcase, World world, List<Text> tooltip, TooltipContext flag) {
		if (briefcase.hasNbt() && briefcase.getNbt().contains("owner"))
			tooltip.add(ClientUtils.localize("tooltip.securitycraft:briefcase.owner", briefcase.getNbt().getString("owner")).setStyle(GRAY_STYLE));
	}

	public static boolean isOwnedBy(ItemStack briefcase, PlayerEntity player) {
		return !briefcase.hasNbt() || !briefcase.getNbt().contains("owner") || briefcase.getNbt().getString("ownerUUID").equals(player.getUuid().toString()) || (briefcase.getNbt().getString("ownerUUID").equals("ownerUUID") && briefcase.getNbt().getString("owner").equals(player.getName().getString()));
	}
}
