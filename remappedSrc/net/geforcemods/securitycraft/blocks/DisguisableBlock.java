...
//	}
	...
	public BlockState getDisguisedBlockState(BlockView world, BlockPos pos)
	{
		if(world.getBlockEntity(pos) instanceof DisguisableTileEntity)
		{
			...
			if(!module.isEmpty() && !((ModuleItem) module.getItem()).getBlockAddons(module.getNbt()).isEmpty())
				return ((ModuleItem) module.getItem()).getBlockAddons(module.getNbt()).get(0).getDefaultState();
		}
	...
	public ItemStack getDisguisedStack(BlockView world, BlockPos pos)
	{
		if(world != null && world.getBlockEntity(pos) instanceof DisguisableTileEntity)
		{
			...
			if(!stack.isEmpty() && !((ModuleItem) stack.getItem()).getBlockAddons(stack.getNbt()).isEmpty())
			{
				ItemStack disguisedStack = ((ModuleItem) stack.getItem()).getAddons(stack.getNbt()).get(0);
