...
public class InventoryScannerFieldBlock extends OwnableBlock implements IIntersectable, BlockEntityProvider {
	...
	private static void redstoneLoop(DefaultedList<ItemStack> inventory, ItemStack stack, InventoryScannerTileEntity te, boolean hasSmartModule, boolean hasStorageModule)
	{
		for(int i = 1; i <= inventory.size(); i++)
		{
			...
			{
				if((hasSmartModule && areItemStacksEqual(itemStackChecking, stack) && ItemStack.areNbtEqual(itemStackChecking, stack))
	...
	public static void checkItemEntity(ItemEntity entity, InventoryScannerTileEntity te, ItemStack stack)
	{
		...
		if(te.hasModule(ModuleType.REDSTONE))
		{
			if((hasSmartModule && areItemStacksEqual(entity.getStack(), stack) && ItemStack.areNbtEqual(entity.getStack(), stack))
	...
	private static boolean checkForShulkerBox(ItemStack item, ItemStack stackToCheck, InventoryScannerTileEntity te, boolean hasSmartModule, boolean hasStorageModule) {
		...
		if(item != null) {
			if(!item.isEmpty() && item.getNbt() != null && Block.getBlockFromItem(item.getItem()) instanceof ShulkerBoxBlock) {
				NbtList list = item.getNbt().getCompound("BlockEntityTag").getList("Items", NbtType.COMPOUND);

				for(int i = 0; i < list.size(); i++) {
					ItemStack itemInChest = ItemStack.fromNbt(list.getCompound(i));
					if((hasSmartModule && areItemStacksEqual(itemInChest, stackToCheck) && ItemStack.areNbtEqual(itemInChest, stackToCheck)) || (!hasSmartModule && areItemStacksEqual(itemInChest, stackToCheck))) {
						list.remove(i);
						deletedItem = true;
