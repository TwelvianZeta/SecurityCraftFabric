...
public class SentryEntity extends PathAwareEntity implements RangedAttackMob //needs to be a creature so it can target a player, ai is also only given to living entities
{
	...
	private static final TrackedData<NbtCompound> MODULE = DataTracker.<NbtCompound>registerData(SentryEntity.class, TrackedDataHandlerRegistry.TAG_COMPOUND);
	private static final TrackedData<NbtCompound> WHITELIST = DataTracker.<NbtCompound>registerData(SentryEntity.class, TrackedDataHandlerRegistry.TAG_COMPOUND);
	...
	public void setupSentry(PlayerEntity owner)
	{
		...
		dataTracker.set(MODULE, new NbtCompound());
		dataTracker.set(WHITELIST, new NbtCompound());
	...
	protected void initDataTracker()
	{
		...
		dataTracker.startTracking(MODULE, new NbtCompound());
		dataTracker.startTracking(WHITELIST, new NbtCompound());
...
//			player.closeHandledScreen(); // TODO
			...
			else if(item == SCContent.DISGUISE_MODULE)
			{
				...
				if(!module.isEmpty()) //drop the old module as to not override it with the new one
				{
					...
					List<Block> blocks = ((ModuleItem)module.getItem()).getBlockAddons(module.getNbt());
			...
			else if(item == SCContent.UNIVERSAL_BLOCK_MODIFIER)
			{
				...
				{
					List<Block> blocks = ((ModuleItem)getDisguiseModule().getItem()).getBlockAddons(getDisguiseModule().getNbt());
...
//			SecurityCraft.channel.send(PacketDistributor.ALL.noArg(), new InitSentryAnimation(getBlockPos(), true, mode == 0));
	...
	{
		if(getMode() != SentryMode.AGGRESSIVE && (target == null && previousTargetId != Long.MIN_VALUE || (target != null && previousTargetId != target.getId())))
		{
...
//			SecurityCraft.channel.send(PacketDistributor.ALL.noArg(), new InitSentryAnimation(getBlockPos(), animate, animateUpwards)); // TODO
	...
	public void writeCustomDataToNbt(NbtCompound tag)
	{
		...
		tag.put("InstalledModule", getDisguiseModule().writeNbt(new NbtCompound()));
		tag.put("InstalledWhitelist", getWhitelistModule().writeNbt(new NbtCompound()));
		tag.putInt("SentryMode", dataTracker.get(MODE));
		tag.putFloat("HeadRotation", dataTracker.get(HEAD_ROTATION));
		super.writeCustomDataToNbt(tag);
	}

	private NbtCompound getOwnerTag()
	{
	...
	{
		NbtCompound teTag = tag.getCompound("TileEntityData");
	...
	{
		List<ItemStack> blocks = ((ModuleItem)module.getItem()).getAddons(module.getNbt());
		...
		dataTracker.set(MODULE, module.writeNbt(new NbtCompound()));
	}
	...
	{
		dataTracker.set(WHITELIST, module.writeNbt(new NbtCompound()));
	}
	...
	{
		NbtCompound tag = dataTracker.get(MODULE);
	...
	{
		NbtCompound tag = dataTracker.get(WHITELIST);
