...
public interface IModuleInventory //extends IItemHandlerModifiable
{
	...
	public default void onModuleInserted(ItemStack stack, ModuleType module)
	{
		...
		if(!te.getWorld().isClient)
			te.getWorld().updateListeners(te.getPos(), te.getCachedState(), te.getCachedState(), 3);
	}
...
//	@Override
	...
	{
		DefaultedList<ItemStack> modules = getInventory();
	 ...
	 */
	public default DefaultedList<ItemStack> readModuleInventory(NbtCompound tag)
	{
		NbtList list = tag.getList("Modules", NbtType.COMPOUND);
		DefaultedList<ItemStack> modules = DefaultedList.ofSize(getMaxNumberOfModules(), ItemStack.EMPTY);
		...
		for(int i = 0; i < list.size(); ++i)
		{
			...
			if(slot >= 0 && slot < modules.size())
				modules.set(slot, ItemStack.fromNbt(stackTag));
		}
	 ...
	 */
	public default NbtCompound writeModuleInventory(NbtCompound tag)
	{
		...
		for(int i = 0; i < modules.size(); i++)
		{
			if(!modules.get(i).isEmpty())
			{
				...
				modules.get(i).writeNbt(stackTag);
				list.add(stackTag);
			}
		}
