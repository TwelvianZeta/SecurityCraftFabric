			...
			if(!player.isCreative())
			{
				...
				for(int i = 1; i <= inventory.size(); i++)
				{
					...
					if(!stackToCheck.isEmpty() && stackToCheck.getItem() instanceof BlockItem)
					{
						...
						if(block instanceof ShulkerBoxBlock && stackToCheck.hasNbt()) //there has to be a check for shulker boxes, otherwise the huge BPs that take 4000 blocks to build couldn't be auto-assembled due to lack of inventory space
						{
							DefaultedList<ItemStack> contents = DefaultedList.<ItemStack>ofSize(27, ItemStack.EMPTY);

							Inventories.readNbt(stackToCheck.getNbt().getCompound("BlockEntityTag"), contents);
				...
				for(int i = 1; i <= inventory.size(); i++) //actually take out the items that are used for assembling the BP
				{
					...
					if(!stackToCheck.isEmpty() && stackToCheck.getItem() instanceof BlockItem)
					{
						...
						if(block instanceof ShulkerBoxBlock && stackToCheck.hasNbt())
						{
							NbtCompound stackTag = stackToCheck.getNbt();
							NbtCompound blockEntityTag = stackTag.getCompound("BlockEntityTag");
							DefaultedList<ItemStack> contents = DefaultedList.<ItemStack>ofSize(27, ItemStack.EMPTY);

							Inventories.readNbt(blockEntityTag, contents);
							...
							Inventories.writeNbt(blockEntityTag, contents);
							stackTag.put("BlockEntityTag", blockEntityTag);
							stackToCheck.setNbt(stackTag);
						} //shulker box end
						else if(block == SCContent.BLOCK_POCKET_WALL)
						{
							if(count <= wallsNeeded)
